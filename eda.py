...
def read_files(path: str) -> pd.DataFrame:
    ...
    for file in tqdm(files):
        ...
        data = pd.concat([data, patient_data])
    return data
...
def plot_sessions(sensor_data:pd.DataFrame, sessions:list, target_cols:list,
    ...
    for session in tqdm(session_subset):
        sensor_data.loc[sensor_data['session_id'] == session]\
            .plot(x='Time', y=target_cols+['AccV', 'AccML', 'AccAP'], figsize=(19.1,9.6), legend=True)
    plt.show()
    plt.clf()
...
plot_sessions(sensor_data, sensor_data['session_id'].unique(), target_cols, 40)
